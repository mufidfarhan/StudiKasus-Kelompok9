<?php
    ...
    $description = $_POST['description'];
		...
		for ($i=0; $i<=2; $i++) {
			...
			if (!in_array($ext, $acc_ext)) {
				...
				echo "<script>
					...
					window.location.href = '../../ninesnkrs-admin/products.php';
				</script>";
			} else {
				if ($size < 1044070) {
					$xx = $rand.'_'.$filename;
					...
					echo "<script>
						...
						window.location.href = '../../ninesnkrs-admin/product-register.php';
					</script>";					
				}
			}
		}
		...
		if ($sql) {
			...
			header("location: ../../ninesnkrs-admin/product-register.php");
			// echo "<script>
